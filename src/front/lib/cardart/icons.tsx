...
/**
 * Карта має власну рамку, а не спільний квадрат 16×16.
 *
 * Решта піктограм — якір, кухоль, надгробок — заповнюють квадрат цілком. Карта
 * ж стоїть сторч, тож у тому самому квадраті займала близько двох третин
 * ширини, а весь малюнок стискався до середини: при однаковому `size` вона
 * виглядала помітно дрібнішою за сусідів, і на 10-11px переставала читатися
 * зовсім.
 *
 * Тут у неї viewBox 12×16 без бічних полів і висота в півтора кегля: у рядку
 * піктограма замінює ціле слово, тож може бути заввишки з сам рядок.
 */
const CARD_ICON_SCALE = 1.5;

function CardSvg({ size = 12, color = "currentColor", title, children }: IconProps & { children: React.ReactNode }) {
  const h = size * CARD_ICON_SCALE;
  return (
    <svg
      viewBox="0 0 12 16"
      width={h * 0.75}
      ...
      role={title ? "img" : "presentation"}
      aria-label={title}
      style={{ display: "inline-block", verticalAlign: "-0.3em", flexShrink: 0 }}
    >
      {title ? <title>{title}</title> : null}
      {children}
    </svg>
...
const CARD_BODY =
  "M2.1 0.5h7.8a1.5 1.5 0 0 1 1.5 1.5v12a1.5 1.5 0 0 1-1.5 1.5H2.1a1.5 1.5 0 0 1-1.5-1.5V2a1.5 1.5 0 0 1 1.5-1.5z";
/** Око на 8.6 одиниць із 10.8 нутра карти: менше воно на 10px зникає. */
const EYE_HOLE = "M1.7 8Q6 3.8 10.3 8Q6 12.2 1.7 8Z";
const PUPIL = "M6 6.45a1.55 1.55 0 1 1 0 3.1a1.55 1.55 0 1 1 0-3.1z";

export const IconFaceUp = (p: IconProps) => (
  <CardSvg {...p} title={p.title ?? "Лицьова карта"}>
 ...
 *
 * Перекреслення — теж проріз, а не лінія поверх: суцільну карту лінією того ж
 * кольору не перекреслиш. У прорізі ока смуга знову перетинає парну кількість
 * меж і стає чорнильною — риска виходить саме там, де вона й потрібна.
 *
 * Смуга завширшки 2.3 одиниці з 12, тобто близько п'ятої частини карти: на
 * 10px це ще два пікселі, і саме вона несе всю різницю між двома значками.
 */
const SLASH_HOLE = "M3.56 13.83L10.36 3.43L8.44 2.17L1.64 12.57Z";

export const IconFaceDown = (p: IconProps) => (
  <CardSvg {...p} title={p.title ?? "Прихована карта"}>
